                        ...
                        <div class="card-header border-0">
                            ...
                            <div class="card-toolbar">
                                ...
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    <!--begin::Admins-->
                                    <button type="button" class="btn btn-primary me-2" onclick="addAdmin()">
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($user); $i++) @php $a=$i; $a++; @endphp 
                                        <tr>
                                            <td>{{$a}}</td>
                                            <td>{{ucwords($user[$i]->first_name)}}</td>
                                            <td>{{ucwords($user[$i]->last_name)}}</td>
                                            <td>{{$user[$i]->email}}</td>
                                            <td>{{$user[$i]->contact_no}}</td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <form  style="display: inline-block" method="POST" action="{{ route('user.destroy', $user[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                <button type="submit" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger me-2" data-bs-toggle="tooltip" data-bs-original-title="Delete Admin">
...
<script type="text/javascript">
    function addAdmin() {
        $.ajax({
            ...
            success: function(result) {
                $('#myModalLgHeading').html('Add Admin');
    ...
    function editAdmin(id) {
        ...
        $.ajax({
            ...
            url: url,
            success: function(result) {
                $('#myModalLgHeading').html('Edit Admin');
                $('#modalBodyLarge').html(result);
                $('#myModalLg').modal('show');
            }
        });
